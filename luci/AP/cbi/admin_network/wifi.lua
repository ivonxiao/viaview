...
function m.parse(map)
	if m:formvalue("cbid.wireless.%s.__toggle" % wdev:name()) then
		...
		luci.http.redirect(luci.dispatcher.build_url("admin/wlan/wireless_status_sole", arg[1]))
		return
	end
	Map.parse(map)
end
...
if hwtype == "mac80211" then
	...
	if cl and #cl > 0 then
		...
		cc = s:option(ListValue, "country", translate("Country Code"))
...
s:option(Value, "ssid", translate("<abbr title=\"Extended Service Set Identifier\">ESSID</abbr>"))
...
if hwtype == "mac80211" then
	...
	hidden = s:option(Flag, "hidden",translate("Hide ESSID"))
...
if has_ap_eap and has_sta_eap and (hwtype == "atheros" or hwtype == "mac80211" or hwtype == "prism2") then
...
s = m:section(NamedSection, wnet.sid, "wifi-iface","MAC Filter Configuration")
ifsection = s
s.addremove = false
s.anonymous = true
s.defaults.device = wdev:name()
