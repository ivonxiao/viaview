...
--	if page.inreq then
		...
		--page.leaf = true
		page = entry({"admin","network","dhcpv4"},template("admin_network/DHCPv4Server"),_("DHCP Server Setting"),20)	
...
function SetACDhcpdConfig()
	...
	local function setDhcpEther(eth)
		--local ethid=eth_cur.eth
		if(eth==2)then	
			eth_cur=info_list.wlan2
		else
			eth_cur=info_list.wlan5
		end	
		if(eth_cur.subnet and eth_cur.mask) then
