...
function index()
	entry({"admin", "system"}, alias("admin", "system", "system"), _("System"), 40).index = true
	entry({"admin", "system", "system"}, cbi("admin_system/system"), _("Basic Setting"), 1)
	entry({"admin", "system", "clock_status"}, call("action_clock_status"))

	entry({"admin", "system", "admin"}, cbi("admin_system/admin"), _("Security"), 2)
	...
	--entry({"admin", "system", "flashops"}, call("action_flashops"), _("Reset / Flash Firmware"), 80)
	
	--entry({"admin", "system", "flashops", "backupfiles"}, form("admin_system/backupfiles"))
	entry({"admin", "system", "reset"}, call("action_reset"), _("Reset"), 70)
	entry({"admin", "system", "firmware"}, call("action_upgrade"), _("Upgrade Firmware"), 50)
	entry({"admin", "system", "flashop"}, call("action_flash"), _("Backup&Restore"), 60)
      ...
      if luci.http.formvalue("image") or luci.http.formvalue("step") then
		...
		if step == 1 then
			if image_supported() then
				...
				luci.template.render("admin_system/upgradeImg", {
					--reset_avail   = reset_avail,
