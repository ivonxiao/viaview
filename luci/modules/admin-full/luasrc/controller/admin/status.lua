...
function index()
	...
	--entry({"admin", "status", "realtime"}, arcombine(template("admin_status/bandwidth"),call("action_bandwidth")), _("RealTime Traffic"), 2)
	entry({"admin", "status", "realtime"}, call("action_traffic"), _("Realtime Graphs"), 2)
	entry({"admin", "status", "realtime", "bandwidth"}, template("admin_status/bandwidth"), _("Traffic"), 1).leaf = true
...
function action_traffic()
	luci.http.redirect(
			luci.dispatcher.build_url("admin", "status", "realtime","bandwidth/")
		)
end
